...
class IscsiQuotas(AbstractModel):
    """
...
class IscsiQuotaSchema(Schema):
    ...
    clients = fields.Int(validate=validate.Range(min=0), required=True)
    data_size_gb = fields.Int(validate=validate.Range(min=0), required=True)
    disks = fields.Int(validate=validate.Range(min=0), required=True)
...
class IscsiQuotaResponseSchema(IscsiQuotaSchema):
    ...
    cluster = fields.Function(lambda quota: quota.get_cluster(), dump_only=True)
