...
def test_permission_member_cannot_change_own_s3user_owner(api, s3_user, headers_factory):
    header = headers_factory.build(aqa_member=True, user=s3_user["owner"])
...
def test_permission_member_cannot_change_s3user_with_different_owner(api, s3_user, headers_factory):
    header = headers_factory.build(aqa_member=True, user="member@example.com")
...
def test_permission_privileged_role_can_change_different_s3users_in_one_account(api, s3_user, headers_factory, case):
    role = case["role"]
    user = case["user"]
    account = case["account"]
    new_owner = "new_owner@example.com"

    header = headers_factory.build(account=account, role=role, user=user)
    payload = S3UserUpdatePayloadFactory.build(owner=new_owner)
    code, data = api.s3.users.update(s3_user["id"], payload, header)
...
def test_admin_and_owner_cannot_change_another_account_s3user(api, s3_user, headers_factory, role):
    header = headers_factory.build(account="devel", role=role)
    payload = S3UserUpdatePayloadFactory.build(use_default=True)
    code, data = api.s3.users.update(s3_user["id"], payload, header)
    assert code == 404, f"Update success: {code} {data}"


def test_operator_can_change_another_account_s3user(api, s3_user, headers_factory):
    header = headers_factory.build(operator=True)
    payload = S3UserUpdatePayloadFactory.build(use_default=True)
    code, data = api.s3.users.update(s3_user["id"], payload, header)
    assert code == 200, f"Update failed: {code} {data}"
...
def test_update_quota_overflow(api, s3_user, headers_factory, quota, expected_message):
...
def test_update_quota_success_for_roles(api, s3_user, headers_factory, role):
    header = headers_factory.build(role=role, user=s3_user["owner"])
    payload = S3UserUpdatePayloadFactory.build(good=True)
    code, data = api.s3.users.update(s3_user["id"], payload, header)
    assert code == 200, f"Update failed: {code} {data}"


def test_update_quota_success_for_operator(api, s3_user, headers_factory):
    header = headers_factory.build(operator=True)
    payload = S3UserUpdatePayloadFactory.build(good=True)
    code, data = api.s3.users.update(s3_user["id"], payload, header)
    assert code == 200, f"Update failed: {code} {data}"
...
def test_update_quota_fail_for_roles(api, s3_user, headers_factory, role):
    header = headers_factory.build(role=role, user=s3_user["owner"])
...
def test_update_quota_fail_for_operator(api, s3_user, headers_factory):
    header = headers_factory.build(operator=True)
    payload = S3UserUpdatePayloadFactory.build(quota={"buckets": 10000, "objects": 10000, "data_size_mb": 10000})
    code, data = api.s3.users.update(s3_user["id"], payload, header)
    assert code == 400, f"Expected 400, got {code}: {data}"
...
def test_update_quota_success_for_roles_in_locked_user(api, locked_s3_user, headers_factory, role):
    header = headers_factory.build(role=role, user=locked_s3_user["owner"])
    payload = S3UserUpdatePayloadFactory.build(good=True)
    code, data = api.s3.users.update(locked_s3_user["id"], payload, header)
    assert code == 200, f"Update failed: {code} {data}"
...
def test_update_combinations_success(case, api, s3_user, headers_factory):
    case.pop("id")
    headers = headers_factory.build()
    code, data = api.s3.users.update(s3_user["id"], case, headers)
    assert code == 200, f"Update failed: {code} {data}"
    try:
        S3UserResponseSchema().load(data)
